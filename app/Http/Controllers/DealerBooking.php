...
class DealerBooking extends Controller{	
		...
		public function dealer_booking(){
            ...
            $rto_date = $report_to_date;
             ...
             if(isset($_GET['dealer_id']))$dealer_id1 = $_GET['dealer_id'];
          	//print $dealer_id1;
            ...
            $dealer_booking_lists = DB::table('dealer_booking')
            			 ...
            			 ->where(function($query) use ($report_from_date, $report_to_date,$dealer_id) {		
							       	...
							       	if($dealer_id !='')
							       		$query->where('dealer_booking.dealer_id', '=', $dealer_id);
        ...
        $total_qty = DB::table('dealer_booking')
                    ->where(function($query) use ($report_from_date, $report_to_date,$dealer_id) {		
							       	 if($report_from_date !='' and $report_to_date !='')
							       	 $query->whereBetween('dealer_booking.booking_date', [$report_from_date, $report_to_date]);
                    ...
                    ->sum('dealer_booking.total_qty');
		
		
		$dealers = DB::table('dealers')->where('is_deleted','=', 0)->get();

			return view('/dealer-booking',compact(['dealer_booking_lists','total_qty','dealers','dealer_id','rfrom_date','rto_date']));
		}
		...
		public function fetch_dealer_stock_amount(){
            ...
            if(isset($_POST['vehicle_type']))$vehicle_type = $_POST['vehicle_type'];
               ...
               $stock_amount = DB::select("SELECT dealer_sale_rate FROM dealer_rate WHERE vehicle_type_id = '$vehicle_type' and vehicle_model_id = '$model_id' and  dealer_id = '$dealer_id' and is_deleted='0'");
		...
		public function print_dealer_receipt($receipt_unique_id,$dealer_unique_id)
		{
            ...
            $print_customer_receipt = DB::table('customer_sales_receipt')
						  ...
						  ->select('customer_sales_receipt.payee_name','customer_sales_receipt.receipt_no','customer_sales_receipt.receipt_date','customer_sales_receipt.amount_to_pay','customer_sales_receipt.payment_mode','customer_sales_receipt.cheque_no','customer_sales_receipt.cheque_bank_name','customer_sales_receipt.credit_card_bank_name','customer_sales_receipt.credit_card_transaction_no','customer_sales_receipt.debit_card_transaction_no','customer_sales_receipt.debit_card_bank_name','customer_sales_receipt.payment_description','dealers.dealer_name','customer_sales_receipt.creator_name') 
		...
		public function dealer_gate_pass_print($dealer_order_id)
		{
			...
			$dealer_booking_vehicle_lists = DB::table('dealer_booking_vehicle_info')
            			 ...
            			 ->where([['dealer_booking_vehicle_info.is_deleted', '=', 0],['dealer_booking_vehicle_info.booking_order_id', '=', $dealer_order_id],['dealer_booking_vehicle_info.return_status', '=', 0],['vehicle_type.is_deleted', '=', 0],['main_model.is_deleted', '=', 0],['colors.is_deleted', '=', 0],['vehicle_stock.is_deleted', '=', 0]])						
						->get();
		...
		public function dealer_receipt($dealer_unique_id){
            ...
            $financial_year = DB::table('financial_year')->get();
             ...
             $start_date = date("Y-m-d", strtotime($financial_year[0]->start_date));
            $end_date = date("Y-m-d", strtotime($financial_year[0]->end_date));
             ...
             $receipt_no1 =  DB::table('customer_sales_receipt') 
            						->where(function($query) use ($start_date, $end_date) {		
							       	 if($start_date !='' and $end_date !='')
							       	 $query->whereBetween('customer_sales_receipt.receipt_date', [$start_date, $end_date]);
							        })
							        ->max('receipt_no');
