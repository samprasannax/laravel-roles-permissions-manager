            ...
            $rto_date = $report_to_date;
             	...
             	$total_finances = DB::table('bank')			 		             
			        ...
			        ->select("bank.bank_name",			        	
			        	DB::raw("(SELECT SUM(amount_to_pay) as total_finance_amount FROM customer_sales_receipt left join sale_booking on sale_booking.order_id = customer_sales_receipt.booking_order_id WHERE sale_booking.hyp_bank_name = bank.id and customer_sales_receipt.finance_status = 1 and customer_sales_receipt.is_deleted = 0 and customer_sales_receipt.cancel_status = 0 and sale_booking.is_deleted = 0 and sale_booking.cancel_status = 0  and customer_sales_receipt.receipt_date between '$report_from_date' and '$report_to_date'  OR customer_sales_receipt.bank_id = bank.id and customer_sales_receipt.finance_status= 1 and customer_sales_receipt.is_deleted=0 and customer_sales_receipt.cancel_status = 0 and customer_sales_receipt.receipt_date between '$report_from_date' and '$report_to_date' ) as total_finance_amount"))
