...
class DailyLedger implements FromCollection
{
    /**
			...
			$voucher_total_amount = DB::table('voucher_receipt')				   
					...
					*/
					//	$find_opening_bal;
						...
						array_push($voucher_report_items,new DailyLedgerReport("","","Report Date: ".$report_from_date,"","","","",""));
						array_push($voucher_report_items,new LedgerReport("","","","","","","",""));

 						array_push($voucher_report_items,new DailyLedgerReport($report_from_date,"Opening Balance","","","","",$find_opening_bal,""));
 						array_push($voucher_report_items,new LedgerReport("","","","","","","",""));
						...
						while(strtotime($report_from_date) <= strtotime($report_to_date)) 
						{
								...
								if(!empty($customer_sales_info_array))
								{
										...
										if($payment_mode1==8)
									{
										$payment_mode = $payee_name." / ".$payment_description." / Road Tax";
								           ...
								           array_push($voucher_report_items,new DailyLedgerReport($receipt_date,$receipt_no,$person_name,"",$amount_to_pay,"",$find_opening_bal,""));
								...
								if(!empty($credit_voucher_array))
								{
										...
										$receipt_no = "Receipt NO:".$cv->voucher_no."/".$payment_mode;
								           ...
								           array_push($voucher_report_items,new DailyLedgerReport($voucher_date,$receipt_no,$person_name,"",$voucher_amount,"",$find_opening_bal,""));
								...
								if(!empty($voucher_receipt_info_array))
								{
									foreach ($voucher_receipt_info_array as $freceipt ) {
								        ...
								        array_push($voucher_report_items,new DailyLedgerReport($voucher_date,"Voucher.No: ".$voucher_no,$person_name,$voucher_name,$voucher_description,$voucher_amount,$find_opening_bal,""));
								        array_push($voucher_report_items,new LedgerReport("","","","","","","",""));
						...
						$report_final = collect($voucher_report_items);
