        ...
        <div class="col-xs-12">
           ...
           <div class="box-header">
              <h3 class="box-title">
               ...
               <form method="post" action="/export_assc_monthly_target_report" style="float:right;padding-right: 15px;">
                                  @csrf
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  @php($count=0)
                  @foreach ($assc_month_target as $asscmt)
                  @php($count++)
                    <tr>
                          <td>{{ $count }}</td>
                          <td>{{ strtoupper($asscmt->dealer_name) }}</td>
                          <td>{{ $asscmt->target_qty}}</td>
                          <td>{{ $asscmt->total_count_scooty }}</td>
                          <td>{{ $asscmt->total_count_motorcycle }}</td>
                          <td>{{ $asscmt->total_count_scooty + $asscmt->total_count_motorcycle }} </td>
                          <td><?php
                          
                            if($asscmt->target_qty != 0)
                            {

                                $data1=$asscmt->target_qty;
                                $dataofdata1=$asscmt->total_count_scooty + $asscmt->total_count_motorcycle;
                          ...
                          <td> <?php
                            ...
                            if($asscmt->target_qty!='0')
                            {
                              $total = $asscmt->total_count_scooty + $asscmt->total_count_motorcycle;
                            echo $asscmt->target_qty - $total;
                            } 
                             ?> </td>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body">
               <form action="/assc_monthly_target_report" method="POST" enctype="multipart/form-data">
