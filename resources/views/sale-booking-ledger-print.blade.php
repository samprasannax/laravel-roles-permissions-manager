...
#vehicle-model-list1 td, #vehicle-model-list1 th {
  border: 1px solid #000;
}

#vehicle-model-list1 tr:nth-child(even){background-color: #f2f2f2;}

#vehicle-model-list1 tr:hover {background-color: #ddd;}

#vehicle-model-list1 th {
  padding-top: 12px;
  padding-bottom: 12px;
  text-align: left; 
  color: #000;
	...
	<div class="main-div">
		 ...
		 <div class="header">
		 	...
		 	<div class="header-left">
		 		...
		 		<p style="font-weight: bold;">Cell : 78789 45658 </p>
		 	</div>
		 	...
		 	<table id="buyer">					
					<tbody>						
						<tr>							
							...
							echo "<br>".$customer_name .",".$swd_category.".".$swd_name."<br>".$customer_address."<br> Contact No: ".$contact_no;
		 ...
		 <div class="body-content">
		 	<table id="vehicle-model-list">
					...
					<tbody>
						...
						<tr>
							<td>2</td>
							<td><b>DSC Name :  {{ $fsbs[0]->sales_person_name }}</b></td>
							<td></td>
						...
						<tr>
							<td>4</td>
							<td><b>Total Amount : {{ $fsbs[0]->grand_total }} </b></td>
							<td></td>
						...
						<tr>
							<td>5</td>
							<td><b>Total Paid Amount : {{ $fsbs[0]->total_paid }} </b></td>
							<td></td>
						...
						<tr>
							<td>6</td>
							<td><b>Total Remaining Amount : {{ $fsbs[0]->total_remaining }} </b></td>
							<td></td>
				...
				<table id="vehicle-model-list1">
					...
					@foreach($fetch_receipt_details as $fs)
					<tr>
					    <td>{{ $fs->receipt_no }}</td>
					    <td>{{ date("d-m-Y", strtotime($fs->receipt_date)) }}</td>
					    <td>{{ $fs->amount_to_pay }}</td>
					    <td>{{ $fs->payee_name }}</td>
					   <td>
                        <?php

                        $payment_mode = $fs->payment_mode;
                        ...
                        if($payment_mode==3)
                        {
                           ...
                           echo" <br> Cheque No : " .  $fs->cheque_no;
                           echo" <br> Bank Name : " .  $fs->cheque_bank_name;
                        }
                        if($payment_mode==4)
                        {
                           echo" <br> Payment Mode : Credit Card";
                           echo" <br> Transaction No : " . $fs->credit_card_transaction_no;
                           echo" <br> Bank Name : " .  $fs->credit_card_bank_name;
                        }
                        if($payment_mode==5)
                        {
                           echo" <br> Payment Mode : Debit Card";
                           echo" <br> Transaction No : " . $fs->debit_card_transaction_no;
                           echo" <br> Bank Name : " . $fs->debit_card_bank_name;
                        }
                        ?>
                      </td>
		 ...
		 <!--	<h3> For SRI SAI KRISHNAS MARKETING PVT LTD </h3>-->
